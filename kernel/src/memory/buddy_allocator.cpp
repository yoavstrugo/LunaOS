...
virtual_address_t k_buddyallocator::allocate(size_t size)
{
    ...
    for (j = requiredOrder; j <= this->maxOrder; j++)
        if (!LINKED_LIST_IS_EMPTY(&this->avail[j]))
            break;
...
void k_buddyallocator::deallocate(virtual_address_t addr)
{
    ...
    if (((virtual_address_t)block < this->base) || ((virtual_address_t)block > (this->base + size)))
        return; // Is the block even in our range
    if (block->tag == 1)
        return; // Block is already available, no need to free it
    ...
    while ((block->kval != this->maxOrder) && (buddy->tag == 1) && (buddy->kval == block->kval))
    {
        ...
        if ((virtual_address_t)buddy < (virtual_address_t)block)
            block = buddy;
