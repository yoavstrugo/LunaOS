...
const k_pci_class_mapping PCI_CLASSCODE_MAPPINGS[] = {
...
k_pci_class_mapping pciGetMapping(uint8_t classCode,
                                  uint8_t subclassCode,
                                  uint8_t progIf)
{
    k_pci_class_mapping mapping;
    for (uint64_t i = 0; i < sizeof(PCI_CLASSCODE_MAPPINGS) / sizeof(k_pci_class_mapping); i++)
    {
        mapping = PCI_CLASSCODE_MAPPINGS[i];
        if (PCI_CLASSCODE_MAPPINGS[i].classCode == classCode &&
            PCI_CLASSCODE_MAPPINGS[i].subclassCode == subclassCode &&
            PCI_CLASSCODE_MAPPINGS[i].progIf == progIf)
            break;
    }
